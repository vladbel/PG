...
namespace PG.LL.Tests
{
    ...
    public class FindLoopStartTests
    {
        ...
        {
            var head = LinkedListNode<int>.Build(new int[] { 0, 1, 2, 3, 4, 5 });
            var loopStart = LinkedListNode<int>.FindFromTail(head, 2);
            var tail = LinkedListNode<int>.GetTail(head);
            tail.Next = loopStart;

            Assert.IsTrue(loopStart.Value == 3);
            Assert.IsTrue(tail.Value == 5);

            var loopStartToTest = LinkedListNode<int>.FindLoopStart(head);
        ...
        {
            var head = LinkedListNode<int>.Build(new int[] { 0, 1, 2, 3, 4, 5 });
            var loopStart = LinkedListNode<int>.FindFromTail(head, 3);
            var tail = LinkedListNode<int>.GetTail(head);
            tail.Next = loopStart;
